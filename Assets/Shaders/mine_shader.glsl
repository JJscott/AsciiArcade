...
layout(location = 3) in mat4 i_modelViewMatrix; // Instanced
layout(location = 7) in float i_explodeTime; // Instanced
...
void main() {
	...
	v_out.norm = m_norm;
	v_out.tex = m_uv;
	v_out.modelViewMatrix = i_modelViewMatrix;
	v_out.explode = i_explodeTime;
}
...
void main() {
	...
	float explode_time = v_in[0].explode;
	mat4 modelViewMatrix = v_in[0].modelViewMatrix;
